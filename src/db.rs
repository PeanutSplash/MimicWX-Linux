        ...
        let text = match self {
            ...
            Self::File {
                ...
                match file_size {
                    ...
                    Some(s) if *s >= 1024 => {
                        format!("[文件] {} ({:.1}KB)", t, *s as f64 / 1024.0)
                    }
...
fn parse_video(content: &str) -> MsgContent {
    ...
    let length = extract_xml_attr(content, "videomsg", "length").and_then(|v| v.parse::<u64>().ok());
    let play_length =
...
fn parse_location(content: &str) -> MsgContent {
    ...
    let scale =
        extract_xml_attr(content, "location", "scale").and_then(|v| v.parse::<u32>().ok());
